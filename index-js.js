...
function pageHidden() {
  ...
  tabCursor.style.visibility = 'hidden';
...
function pageVisibility() {
  ...
  tabCursor.style.visibility = 'visible';
  setTimeout(() => {
    cookie.classList.remove('close');
  }, 2000);
...
const macbook__display = document.querySelector('.macbook__display');
...
const labelSpans = document.querySelectorAll('#pageFerst .label span');
let indexLabel = 0;
...
const cardTg = document.querySelector('.informationBox .cardTg');
const plane = document.querySelector('.informationBox .plane');
const tgAv = plane.getAttribute('data-av');

const qrcodTg = document.querySelector('.informationBox .qrcodTg');
const tgUrl = qrcodTg.getAttribute('data-tg');

plane.addEventListener('click', () => {
  window.open(tgAv, '_blank')
})

cardTg.addEventListener('click', () => {
...
const gameContainerText = document.querySelector('.gameContainerBox .textBlock');
// const promocodBlock = document.querySelector('.gameContainerBox .promocodBlock');
const promocodText = document.querySelector('.gameContainerBox .promocodText');

const gameTextPromo = document.querySelector('.gameContainerBox .textBlock .promo');
const gameTextStart = document.querySelector('.gameContainerBox .textBlock .start');

gameContainerBox.addEventListener('mouseenter', () => {
  gameTextPromo.style.opacity = 0
  gameTextStart.style.opacity = 1
})

gameContainerBox.addEventListener('mouseleave', () => {
  gameTextPromo.style.opacity = 1
  gameTextStart.style.opacity = 0
})

// gameContainerBox.addEventListener('mouseenter', () => {
//   gameTextStart.classList.add('open');
//   gameTextPromo.classList.remove('open');
// })

// gameContainerBox.addEventListener('mouseleave', () => {
//     gameTextPromo.classList.add('open');
//   gameTextStart.classList.remove('open');
// })
...
gameContainerBox.addEventListener('click', () => {
  if (!containerGameOpen) {
    if (playIsPassed) {
      // promocodText.style.display = 'flex';
    }
    gameOpenFun = true;
    setTimeout(() => {
...
exitPlay.addEventListener('click', () => {
  ...
  if (containerGameOpen) {
    ...
    body.style.overflowY = 'auto';
    // gameContainerText.style.display = 'flex'
    gameContainerText.classList.remove('notShowEase')
    // promocodText.style.display = 'none';



    exitPlay.style.display = 'none';
    // exitMaxPlay.style.display = 'none';
    // exitMaxPlay.style.opacity = 0;

    exitMaxPlay.classList.remove('open');
...
function gameA() {
  ...
  function endGame(success) {
    ...
    gameContainer.style.background = '#444'
      ...
      if (forwardIndex >= stateC.recordedFrames.length) {
        ...
        setTimeout(() => {
          gameContainers.forEach(gameContainer => {
            ...
            gameContainer.querySelector('.door').style.display = 'none';
            gameContainer.querySelectorAll('.wall').forEach(wall => {
              wall.style.display = 'none';
            });

            gameContainerBox.style.backgroundColor = '#444'
            gameContainerText.style.display = 'none'
          });

          setTimeout(() => {
            promocodText.classList.add('open')
            playIsPassed = true;
          }, 2000);
...
let goToSponsors = false;
const sponsors = document.querySelector('.sponsors');

// const sponsorsPart = document.querySelectorAll('.sponsors .part');
const sponsorsPart = document.querySelectorAll('.listCompany .part');
// const listCompanyPart = document.querySelectorAll('.listCompany .part');
...
sponsorsPart.forEach(part => {
  const info = part.querySelector('.listCompany .part .info');
  ...
  part.addEventListener('click', () => {
    ...
    if (part.classList.contains('PaWell')) {
      const sponsorsOther = part.querySelector('.listCompany .part .other');
      sponsorsOther.style.display = 'none'

    }
    else {
      const sponsorsSvg = part.querySelector('.listCompany .part svg');
      sponsorsSvg.style.display = 'none'
    }
    part.classList.add('infoOpen')

    tabCursor.classList.add('black')
  ...
  part.addEventListener('mouseleave', () => {
    ...
    if (part.classList.contains('PaWell')) {
      const sponsorsOther = part.querySelector('.listCompany .part .other');
      sponsorsOther.style.display = 'block'

    }
    else {
      const sponsorsSvg = part.querySelector('.listCompany .part svg');
      sponsorsSvg.style.display = 'block'
    }
    part.classList.remove('infoOpen')

    tabCursor.classList.remove('black')
...
sponsorsPart.forEach(link => {
  ...
  link.addEventListener('click', function (e) {
    if (goToSponsors) {
      ...
      setTimeout(() => {
        macbook__display.classList.add('add404')
...
const cookieB = document.querySelector('.cookie');
const cookieExit = document.querySelector('.cookie .exit');


cookieExit.addEventListener('click', () => {
  cookieB.classList.add('close')
})
...
document.querySelectorAll('#pageMain .grid > div').forEach(item => {
  // if (item.classList.contains('gameContainerBox')) return;exitMax
  if (item.classList.contains('exitMax')) return;
