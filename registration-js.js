...
const confirmInputs = document.querySelectorAll('.blockConfirmEmail .code-input');
...
formCreate.addEventListener('submit', function (e) {
    ...
    if (isValid) {
        ...
        blockConfirmEmail.classList.toggle('notShow')
...
const blockConfirmEmail = document.querySelector('.blockConfirmEmail')
const formConfirm = document.querySelector('#code-inputs')
...
const exitMaxConfirmEmail = document.querySelector('.blockConfirmEmail .exitMax')
exitMaxConfirmEmail.addEventListener('click', function () {

    blockConfirmEmail.classList.toggle('notShow')
    body.style.overflow = 'auto';
})

exitMaxConfirmEmail.addEventListener('mouseenter', function () {

    exitConfirmEmail.classList.add('mouseenter');
})
exitMaxConfirmEmail.addEventListener('mouseleave', function () {

    exitConfirmEmail.classList.remove('mouseenter');
})




const exitConfirmEmail = document.querySelector('.blockConfirmEmail .exit')
exitConfirmEmail.addEventListener('click', function () {
...
confirmInputs.forEach((input, index) => {
    ...
    input.addEventListener('input', (e) => {
        ...
        checkCodeComplete(); // Проверка заполнения всего кода
    });
...
confirmInputs[0].addEventListener('paste', (e) => {
    e.preventDefault();
    const pastedData = e.clipboardData.getData('text');
    const digits = pastedData.replace(/\D/g, '').split('').slice(0, 6);

    digits.forEach((digit, index) => {
        if (confirmInputs[index]) {
            confirmInputs[index].value = digit;
        }
    });

    if (digits.length === 6) {
        confirmInputs[5].focus();
        checkCodeComplete();
    }
});
...
// submitConfirm.addEventListener('submit', function (e) {
formConfirm.addEventListener('submit', function (e) {
    ...
    if (codeIsCorrect) {
        ...
        confirmInputs.forEach(input => {
            showError(input, 'formConfirm-error', 'Неверно введен код');
...
// formConfirm.addEventListener('input', function () {
//     hideCorrect(this);
//     showDelete('emailForgot-delete');
...
//     }
// });
// formConfirm.addEventListener('blur', function () {
//     const nameRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
//     hideCorrect(this);
